...
원천 데이터는 공공데이터포털(data.go.kr)과 서울 열린데이터광장(data.seoul.go.kr)에서
받았다. 재배포 시 각 제공기관의 이용조건(공공누리 유형 포함)을 따라야 하며,
스냅샷 바이트를 게시하기 전에 데이터셋별 유형을 확인해야 한다. 코드의 라이선스는
[`LICENSE`](LICENSE)에 있고, 데이터에는 적용되지 않는다.
