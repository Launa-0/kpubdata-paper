...
- 이 규칙은 **결과를 확인하기 전에 사전 정의(pre-specified)했다.** 작업용 분석 계획
  (`overnight/rq1-transition/PRE_ANALYSIS.md` 11절, 파일 시각 2026-09-24 02:24)에 먼저 적고,
  그 문서를 따르는 분류 스크립트는 03:03에, 첫 결과는 03:05에 만들어졌다. 근거는 로컬 파일
  시각이다 — 공개 레지스트리나 외부 타임스탬프에 등록한 것이 아니므로 formal preregistration이라
  부르지 않는다. 작업용 계획 문서는 저장소에 올리지 않고, 규칙만 이 문서로 옮겼다.
- 결과를 본 뒤 **범주 정의를 바꾸지 않았다.** 이후 측정 코드의 날짜 투영 버그를 고쳐 다시
  돌렸을 때도 규칙은 그대로였고, 규칙 4(`derived_field`)가 실제로 적용되기 시작했을 뿐이다.
- 하네스 구현(`pair_and_classify`)은 그 분류 스크립트와 별개로 다시 작성했다. 서울 3종 전수에서
  role × 원인 60칸이 그 스크립트의 결과와 **모두 같았다** (불일치 0).
...
규칙 5와 7의 `strip()`은 사전 정의한 그대로다. 측정한 스냅샷에서 이 `strip()`에 걸리는 값은
없다.
