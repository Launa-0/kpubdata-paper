r"""서울 공공자전거 월별 이용정보의 Silver 계약 (RQ1, R2 #18).
...
`3` / `00003` / `102`         zfill. 대여소번호는 required 식별자라 폭이 맞아야 한다.
...
SPEC: dict[str, Any] = {
    ...
    "contract": {
        ...
        # 동시에 나오고, 통합 스냅샷 기준으로 빈 문자열 쪽이 더 많다(1,040,784 vs 940,297).
        # 전역 null_tokens에 ""를 넣으면 **모든 문자열 컬럼**의 빈 문자열을 결측으로
        # 해석하게 된다. 통합 스냅샷에서는 빈 문자열이 성별에서만 관측되어 결과가 같지만,
